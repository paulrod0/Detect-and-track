...
function selectTask(task) {
    for (let i = 0; i < selectedArr.length; i++) {
        if (task == i)
            selectedArr[i] = false; // cambiar a true
        else
            selectedArr[i] = false;
    }
}
...
function randomId(length) {
    let chars = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'; // de momento  si, luego habría que ver si sería conveniente añadir algún otro caracter
    var result = '';
